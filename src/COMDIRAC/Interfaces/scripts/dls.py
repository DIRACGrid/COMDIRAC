    ...
    class ReplicaFileCatalogClientCLI(FileCatalogClientCLI):
        ...
        def do_ls(self, args):
            ...
            if not result["OK"]:
                ...
                path in result["Value"]["Successful"]
                ...
                dList.addFileWithReplicas(
                    os.path.basename(path), fileDict, numericid, replicas
                )
            ...
            if not result["OK"]:
                ...
                path not in result["Value"]["Successful"]
                or not result["Value"]["Successful"][path]
            ):
                ...
                if result["OK"]:
                    if result["Value"]["Successful"]:
                        for entry in result["Value"]["Successful"][path]["Files"]:
                            ...
                            if long:
                                fileDict = result["Value"]["Successful"][path]["Files"][
                                    entry
                                ]["MetaData"]
                                if fileDict:
                                    ...
                                    dList.addFileWithReplicas(
                                        fname, fileDict, numericid, replicas
                                    )
                            else:
                        ...
                        for entry in result["Value"]["Successful"][path]["SubDirs"]:
                            ...
                            if long:
                                dirDict = result["Value"]["Successful"][path][
                                    "SubDirs"
                                ][entry]
                                if dirDict:
